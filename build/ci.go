...
var (
	// Files that end up in the netk*.zip archive.
	netkArchiveFiles = []string{
		"COPYING",
		executablePath("netk"),
	}

	// Files that end up in the netk-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			Name:        "abigen",
			Description: "Source code generator to convert NetworkChain contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			Name:        "bootnode",
			Description: "NetworkChain bootnode.",
		},
		{
			Name:        "evm",
			Description: "Developer utility version of the EVM (NetworkChain Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			Name:        "netk",
			Description: "NetworkChain CLI client.",
		},
		{
			Name:        "puppeth",
			Description: "NetworkChain private network manager.",
		},
		{
			Name:        "rlpdump",
		...
		{
			Name:        "swarm",
			Description: "NetworkChain Swarm daemon and tools",
		},
		{
			Name:        "wnode",
			Description: "NetworkChain Whisper diagnostic tool",
		},
	}
...
func doArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archives (usually "netkstore/builds")`)
		ext    string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		...
		netk     = "netk-" + base + ext
		alltools = "netk-alltools-" + base + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(netk, netkArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{netk, alltools} {
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Where to upload the source package (usually "ppa:networkchain/networkchain")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "netk-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func newDebMetadata(distro, author string, env build.Environment, t time.Time) debMetadata {
	if author == "" {
		...
		author = "NetworkChain Builds <fjl@networkchain.io>"
	}
	return debMetadata{
		Env:         env,
		Author:      author,
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "netk.exe" {
			netkTool = file
		} else {
	...
	// first section contains the netk binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Netk":     netkTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.netk.nsi", filepath.Join(*workdir, "netk.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("netk-" + archiveBasename(*arch, env) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "netk.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archive (usually "netkstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	...
	if *local {
		...
		os.Rename("netk.aar", filepath.Join(GOBIN, "netk.aar"))
		return
	}
	...
	archive := "netk-" + archiveBasename("android", env) + ".aar"
	os.Rename("netk.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	archive := "netk-" + archiveBasename("ios", env)
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Netk.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Netk.podspec", "--allow-warnings", "--verbose")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "netkstore/builds")`)
