...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the NetworkChain protocol")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for NetworkChain and the faucet")
)
...
type request struct {
	...
	Account  common.Address     `json:"account"`  // NetworkChain address being funded
	Time     time.Time          `json:"time"`     // Timestamp when te request was accepted
	Tx       *types.Transaction `json:"tx"`       // Transaction funding the account
}

// faucet represents a crypto faucet backed by an NetworkChain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // NetworkChain protocol stack
	client *ethclient.Client   // Client connection to the NetworkChain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the NetworkChain light client protocol
	...
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightNetworkChain
			ctx.Service(&serv)
...
// close terminates the NetworkChain connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Iterate over all the files and look for NetworkChain addresses
		...
		if address == (common.Address{}) {
			websocket.JSON.Send(conn, map[string]string{"error": "No NetworkChain address found to fund"})
			continue
		}
