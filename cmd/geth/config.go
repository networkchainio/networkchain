...
import (
	...
	"github.com/networkchain/go-networkchain/contracts/release"
...
func loadConfig(file string, cfg *netkConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "netk.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, netkConfig) {
	// Load defaults.
	cfg := netkConfig{
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	// Add the NetworkChain Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		utils.Fatalf("Failed to register the Netk release oracle service: %v", err)
	}
	return stack
}
