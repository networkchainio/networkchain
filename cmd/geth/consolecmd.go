...
// localConsole starts a new netk node, attaching a JavaScript console to it at the
// same time.
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Failed to attach to the inproc netk: %v", err)
	}
...
// remoteConsole will connect to a remote netk instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	// Attach to a remotely running netk instance and start the JavaScript console
	client, err := dialRPC(ctx.Args().First())
	if err != nil {
		utils.Fatalf("Unable to attach to remote netk: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.GlobalString(utils.JSpathFlag.Name),
...
// for "netk attach" and "netk monitor" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint(clientIdentifier)
	} else if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with netk < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
...
// ephemeralConsole starts a new netk node, attaches an ephemeral JavaScript
