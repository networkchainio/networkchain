...
const (
	clientIdentifier = "netk" // Client identifier to advertise over the network
)
...
// netk is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func netk(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) {
		// Mining only makes sense if a full NetworkChain node is running
		var networkchain *eth.NetworkChain
		if err := stack.Service(&networkchain); err != nil {
			utils.Fatalf("networkchain service not running: %v", err)
		}
		...
		if threads := ctx.GlobalInt(utils.MinerThreadsFlag.Name); threads > 0 {
			...
			if th, ok := networkchain.Engine().(threaded); ok {
				th.SetThreads(threads)
			}
		}
		// Set the gas price to the limits from the CLI and start mining
		networkchain.TxPool().SetGasPrice(utils.GlobalBig(ctx, utils.GasPriceFlag.Name))
		if err := networkchain.StartMining(true); err != nil {
