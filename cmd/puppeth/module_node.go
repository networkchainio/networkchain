...
// nodeDockerfile is the Dockerfile required to run an NetworkChain node.
var nodeDockerfile = `
FROM networkchain/client-go:alpine-develop
...
{{end}}
RUN \
  echo '/netk init /genesis.json' > netk.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.networkchain/keystore/ && cp /signer.json /root/.networkchain/keystore/' >> netk.sh && \{{end}}
	echo $'/netk --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .Etherbase}}--etherbase {{.Etherbase}} --mine{{end}}{{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> netk.sh

ENTRYPOINT ["/bin/sh", "netk.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an NetworkChain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new NetworkChain node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 /netk --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.networkchain"],
