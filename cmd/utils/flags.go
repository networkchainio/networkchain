...
// for Netk and returns half of the allowance to assign to the database.
...
func SetNodeConfig(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(DevModeFlag.Name):
		cfg.DataDir = filepath.Join(os.TempDir(), "networkchain_dev_mode")
	case ctx.GlobalBool(TestnetFlag.Name):
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	// NetworkChain needs to know maxPeers to calculate the light server peer ratio.
	// TODO(fjl): ensure NetworkChain can get MaxPeers from node.
	cfg.MaxPeers = ctx.GlobalInt(MaxPeersFlag.Name)
...
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the NetworkChain service: %v", err)
	}
}
...
// RegisterEthStatsService configures the NetworkChain Stats daemon and adds it to
// th egiven node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		Fatalf("Failed to register the NetworkChain Stats service: %v", err)
	}
}
...
// new format.
//
// e.g. netk account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// netk --keystore /tmp/mykeystore --lightkdf account new
//
