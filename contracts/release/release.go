...
type Config struct {
	Oracle common.Address // NetworkChain address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the NetworkChain service dependency to access the blockchain
	var apiBackend ethapi.Backend
	var networkchain *eth.NetworkChain
	if err := ctx.Service(&networkchain); err == nil {
		apiBackend = networkchain.ApiBackend
	} else {
		var networkchain *les.LightNetworkChain
		if err := ctx.Service(&networkchain); err == nil {
			apiBackend = networkchain.ApiBackend
		} else {
			return nil, err
