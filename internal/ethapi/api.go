...
// PublicNetworkChainAPI provides an API to access NetworkChain related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicNetworkChainAPI struct {
	b Backend
}

// NewPublicNetworkChainAPI creates a new Etheruem protocol API.
func NewPublicNetworkChainAPI(b Backend) *PublicNetworkChainAPI {
	return &PublicNetworkChainAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicNetworkChainAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current NetworkChain protocol version this node supports
func (s *PublicNetworkChainAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicNetworkChainAPI) Syncing() (interface{}, error) {
...
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19NetworkChain Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an NetworkChain ECDSA signature for:
// keccack256("\x19NetworkChain Signed Message:\n" + len(message) + message))
//
...
// the address of:
// hash = keccak256("\x19NetworkChain Signed Message:\n"${message length}${message})
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid NetworkChain signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the NetworkChain blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
...
//
// https://github.com/networkchain/wiki/wiki/JSON-RPC#eth_sign
...
// Version returns the current networkchain protocol version.
