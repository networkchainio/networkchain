...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light NetworkChain protocol")
	...
	log.Info("Light NetworkChain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := p.Handshake(td, head, headNum, genesis, pm.server); err != nil {
		p.Log().Debug("Light NetworkChain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light NetworkChain peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light NetworkChain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light NetworkChain message arrived", "code", msg.Code, "bytes", msg.Size)
