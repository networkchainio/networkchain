...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Netk.LightScryptN, Netk.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.networkchain", new NodeConfig());
			node.start();
			...
			NetworkChainClient ec = node.getNetworkChainClient();
	...
	// signatures too. Regression test for go-networkchain issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altonetker
	...
	workspace, err := ioutil.TempDir("", "netk-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/networkchain/netktest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Netk and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.networkchain", "github.com/networkchain/go-networkchain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "netk.aar"), "netk.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "networkchain", "netktest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.networkchain.netktest"
