...
// Package eth implements the NetworkChain protocol.
package eth
...
import (
	...
	"github.com/networkchain/go-networkchain/consensus/clique"
	...
	"github.com/networkchain/go-networkchain/miner"
...
type NetworkChain struct {
	...
	shutdownChan  chan bool // Channel for shutting down the networkchain
...
func (s *NetworkChain) AddLesServer(ls LesServer) {
	s.lesServer = ls
}

// New creates a new NetworkChain object (including the
// initialisation of the common NetworkChain object)
func New(ctx *node.ServiceContext, config *Config) (*NetworkChain, error) {
	if config.SyncMode == downloader.LightSync {
		return nil, errors.New("can't run eth.NetworkChain in light sync mode, use les.LightNetworkChain")
	}
	...
	log.Info("Initialising NetworkChain protocol", "versions", ProtocolVersions, "network", config.NetworkId)

	if !config.SkipBcVersionCheck {
		bcVersion := core.GetBlockChainVersion(chainDb)
		if bcVersion != core.BlockChainVersion && bcVersion != 0 {
			return nil, fmt.Errorf("Blockchain DB version mismatch (%d / %d). Run netk upgradedb.\n", bcVersion, core.BlockChainVersion)
		}
...
// CreateConsensusEngine creates the required type of consensus engine instance for an NetworkChain service
...
func (s *NetworkChain) APIs() []rpc.API {
	...
	return append(apis, []rpc.API{
		{
			Namespace: "eth",
			Version:   "1.0",
			Service:   NewPublicNetworkChainAPI(s),
			Public:    true,
		}, {
			Namespace: "eth",
			Version:   "1.0",
...
func (s *NetworkChain) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *NetworkChain) Etherbase() (eb common.Address, err error) {
	s.lock.RLock()
...
func (self *NetworkChain) SetEtherbase(etherbase common.Address) {
	self.lock.Lock()
...
func (s *NetworkChain) StartMining(local bool) error {
	eb, err := s.Etherbase()
	if err != nil {
...
func (s *NetworkChain) StopMining()         { s.miner.Stop() }
func (s *NetworkChain) IsMining() bool      { return s.miner.Mining() }
func (s *NetworkChain) Miner() *miner.Miner { return s.miner }

func (s *NetworkChain) AccountManager() *accounts.Manager  { return s.accountManager }
func (s *NetworkChain) BlockChain() *core.BlockChain       { return s.blockchain }
func (s *NetworkChain) TxPool() *core.TxPool               { return s.txPool }
func (s *NetworkChain) EventMux() *event.TypeMux           { return s.eventMux }
func (s *NetworkChain) Engine() consensus.Engine           { return s.engine }
func (s *NetworkChain) ChainDb() ethdb.Database            { return s.chainDb }
func (s *NetworkChain) IsListening() bool                  { return true } // Always listening
func (s *NetworkChain) EthVersion() int                    { return int(s.protocolManager.SubProtocols[0].Version) }
func (s *NetworkChain) NetVersion() uint64                 { return s.networkId }
func (s *NetworkChain) Downloader() *downloader.Downloader { return s.protocolManager.downloader }

// Protocols implements node.Service, returning all the currently configured
// network protocols to start.
func (s *NetworkChain) Protocols() []p2p.Protocol {
	if s.lesServer == nil {
...
func (s *NetworkChain) Start(srvr *p2p.Server) error {
...
func (s *NetworkChain) Stop() error {
