...
import (
	...
	"github.com/networkchain/go-networkchain/consensus/misc"
	...
	"github.com/networkchain/go-networkchain/eth/fetcher"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing NetworkChain peer", "peer", id)

	// Unregister the peer from the downloader and NetworkChain peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping NetworkChain protocol")
	...
	log.Info("NetworkChain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("NetworkChain peer connected", "name", p.Name())
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("NetworkChain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("NetworkChain message handling failed", "err", err)
			return err
		}
	}
}
...
// EthNodeInfo represents a short summary of the NetworkChain sub-protocol metadata known
// about the host peer.
type EthNodeInfo struct {
	Network    uint64      `json:"network"`    // NetworkChain network ID (1=Frontier, 2=Morden, Ropsten=3)
