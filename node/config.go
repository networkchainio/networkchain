...
import (
	...
	"github.com/networkchain/go-networkchain/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of netk is "netk". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Netk", keep that.
	if name == "netk" || name == "netk-testnet" {
		name = "Netk"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "netk" instances.
var isOldNetkResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by netk 1.4 are used if they exist.
	if c.name() == "netk" && isOldNetkResource[path] {
		oldpath := ""
		if c.Name == "netk" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	default:
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-networkchain-keystore")
		ephemeral = keydir
	}
	if err != nil {
		return nil, "", err
