...
import (
	...
	"github.com/networkchain/go-networkchain/logger"
	"github.com/networkchain/go-networkchain/p2p"
	"github.com/networkchain/go-networkchain/p2p/nat"
	"github.com/networkchain/go-networkchain/whisper"
)
...
func main() {
	...
	// Create an NetworkChain peer to communicate through
	server := p2p.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
	...
	fmt.Println("Starting NetworkChain peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start NetworkChain peer: %v.\n", err)
		os.Exit(1)
	}
